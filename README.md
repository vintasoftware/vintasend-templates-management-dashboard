...
version of each key by default; tick *Show every version* for the raw listing.
...
means *the latest* everywhere except the status-history endpoint, where it means
*every version*. Every write here names the version explicitly, because the row
